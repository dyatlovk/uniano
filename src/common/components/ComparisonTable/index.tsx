  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.top_wrapper}>
        ...
        <div
          style={{ opacity: switchButton && !isSwitchActive2 ? 0.3 : 1 }}
          className={styles.details_box}
        >
          {card2}
          <div className={styles.absolute_switch}>
            {switchButton && (
              <SwitchButton callback={item => setIsSwitchActive2(item)} />
            )}{' '}
          </div>
        </div>
        <div
          style={{ opacity: switchButton && !isSwitchActive3 ? 0.3 : 1 }}
          className={styles.details_box}
        >
          {card3}
          <div className={styles.absolute_switch}>
            {switchButton && (
              <SwitchButton callback={item => setIsSwitchActive3(item)} />
            )}{' '}
          </div>
        </div>
      </div>
      ...
      <div className={styles.bottom_wrapper}>
        {rows.map((row, index) => (
          <div
            ...
            style={{
              backgroundColor: index % 2 != 0 ? 'white' : AppColor.white,
            }}
          >
            ...
            <div
              style={{
                opacity: switchButton && !isSwitchActive2 ? 0.3 : 1,
                pointerEvents: !isSwitchActive2 ? 'none' : 'auto',
              }}
              className={styles.bottom_row_item}
            >
              <RowItem item={row.card2Text} />
            </div>
            <div
              style={{
                opacity: switchButton && !isSwitchActive3 ? 0.3 : 1,
                pointerEvents: !isSwitchActive2 ? 'none' : 'auto',
              }}
              className={styles.bottom_row_item}
            >
  ...
  const typeComponentMap = {
    ...
    boolean: () => (
        ...
        style={
          !isChecked
            ? {
                ...
                boxShadow: '-1px 1px 6px 2px rgba(0,0,0,0.11)',
              }
            : {}
        }
        onClick={() => {
      ...
      >
        {' '}
        {isChecked && (
          <AppColor.singTrue stroke="white" width={14} height={10} />
        )}{' '}
      </div>
    ),
  }
