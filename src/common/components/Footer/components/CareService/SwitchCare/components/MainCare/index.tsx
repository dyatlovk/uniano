    ...
    <div className={styles.main_care_grid}>
      {authorisedUser && (
        <>
          <div
            onClick={() => {
              setActiveSwitch('main.solutions nopadding')
            }}
          >
            <CareServiceItem
              description="Projects & Sponsorships"
              title="Solutions"
          ...
          <div
            onClick={() => {
              setActiveSwitch('main.direct')
            }}
          >
            <CareServiceItem
              description="Communication With Other "
              title="Direct  "
      ...
      {!authorisedUser && (
        <>
          <div
            onClick={() => {
              setActiveSwitch('main.community')
            }}
          >
            <CareServiceItem
          ...
          <div
            onClick={() => {
              setActiveSwitch('main.contact us')
            }}
          >
            <CareServiceItem
...
const CareServiceItem = ({
  ...
  icon,
  image,
}: CareServiceItemProps) => {
  return (
    <div
      style={{ backgroundImage: `url(${image ?? test1})` }}
      className={`${styles.care_service_item} cursor`}
    >
      <div style={{ zIndex: 1 }}>
        <Typography
          variant="body4"
          className={styles.care_text}
          fontWeight="500"
        >
          {title}
        </Typography>
        <SizeBox height="5px" />
        <Typography variant="body5" className={styles.care_text}>
          {description}
        </Typography>
      </div>
      <div className={styles.background_dark}></div>
      <div className={styles.care_service_abs}>{icon}</div>
    </div>
  )
}
