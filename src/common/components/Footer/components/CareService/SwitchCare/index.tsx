...
export const CareComponent = ({ showHelper, callback }: CareComponentProps) => {
  ...
  const lastTextItem =
    activeSwitch.split(' ')[activeSwitch.split(' ').length - 1]
  const lastItem = activeSwitch
    .split('.')
    [activeSwitch.split('.').length - 1].split(' ')
          ...
          <div className={styles.helper_padding}>
            ...
            >
              <div className="gap_15">
                {activeSwitch != 'main' && (
                  <div onClick={moveBack} className="cursor">
...
const SwitchCare = ({ activeSwitch, setActiveSwitch }: SwitchCareProps) => {
  ...
  const returnMap = {
    ...
    'main.community.getting started': (
      <GettingsStarted
        setActiveSwitch={item => {
          setActiveSwitch(item)
        }}
