...
const HeaderNothAuthorized = () => {
  ...
  return (
    <div className={styles.header_fixed}>
      <div
        className={`${styles.header_modal_fixed} ${showModal && styles.active_modal}`}
      >
      ...
      <div style={{ opacity: visible ? '1' : '0' }} className={styles.wrapper}>
        <header
          style={{ backgroundColor: showModal ? '#EFEFEF' : 'transparent' }}
        ...
        >
          <div className={styles.header__container}>
            <div
              ...
              style={{
                backgroundColor: showModal ? AppColor.orange : 'transparent',
              }}
              className={styles.hamburger__wrapper}
            >
              <span
                style={{
                  backgroundColor: showModal ? 'white' : AppColor.orange,
                }}
            ...
            <div
              ...
              className={styles.header_second_part}
            >
              <li>
                <ul title="Customers" {...eventHandlers}>
                  Customers
                </ul>
                <ul title="Freelancers" {...eventHandlers}>
                  Freelancers
                </ul>
                <ul title="Sponsors" {...eventHandlers}>
                  Sponsors
                </ul>
                <ul title="Managers" {...eventHandlers}>
                  Managers
                </ul>
              </li>
              <div
                {...eventHandlers}
                className={`${styles.hover_wrapper} ${hovered ? styles.active : styles.disabled}`}
              >
                <MenuLinks />
              </div>
            </div>
...
export const MobileNavBar = ({
  ...
  const [activeCategory, setActiveCategory] = useState<categorysText>(
  ...
  return (
    <div className={styles.active_selection}>
      {prevSelection != '' && (
        <div
          onClick={() => {
            callbackSelection(prevSelection)
          }}
          className={`${styles.active_selection_padding} gap_10`}
        >
...
const ChooseSubCategory = ({
  ...
  return (
    <div
      ...
      style={{
        backgroundColor:
          activeTitle == details.title ? AppColor.white : 'transparent',
      }}
    >
      {details.icon}
      <Typography
        variant="body4"
        fontWeight={activeTitle == details.title ? '500' : '400'}
      >
        {details.title}
      </Typography>
    </div>
...
export const ChooseCategory = () => {
  const [activeCategory, setActiveCategory] = useState<ChooseCategoryType>(
    categoryItems[0]
  )
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div
        ...
        className={`${styles.choose_category_item} cursor`}
      >
      ...
      <div
        ...
        className={styles.abs_trianle_center}
      ></div>
      ...
      <div
        ...
        className={styles.choose_category_grid}
      >
        {categoryItems.map(item => (
          <div
            style={{ padding: '5px 0px', alignItems: 'stretch' }}
            onClick={() => {
              setActiveCategory(item)
              setShowdropdown(false)
            }}
            className="gap_10 cursor"
          >
            {item.icon}
...
export const SelectDropdown = ({
  ...
  return (
    <div onClick={handleClick} className={styles.select_dropdown}>
      <Typography
        color={activeTitle == title ? AppColor.orange : AppColor.text}
        fontSizeStatic="1rem"
        fontWeight="400"
      >
        {title}
      ...
      <AppColor.chevronBottom
        fill={activeTitle == title ? AppColor.orange : AppColor.text}
        width={'12px'}
        height={'6px'}
      />
    </div>
