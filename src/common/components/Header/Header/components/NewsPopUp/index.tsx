...
const topFilters = ['Roadmap', 'Activity', 'Saved & Notes', 'Templates']
const NewsPopUp = () => {
    ...
    <div className={styles.news_wrapper}>
      <div className={styles.top_filter}>
        {topFilters.map(item => (
          <div
            style={{
              borderBottom: `2px solid ${item == filterTop ? AppColor.orange : AppColor.transparent}`,
            }}
          ...
          >
            <Typography
              variant="body4"
              fontWeight={item == filterTop ? '500' : '400'}
              color={item == filterTop ? AppColor.orange : AppColor.text}
            >
              {item}
            </Typography>
          </div>
      ...
      <div>
        <MapItemBottom
          activeBottomSelect={activeFilterBottom}
          callbackBottom={item => {
            setActiveFilterBottom(item)
          }}
          activeSelection={filterTop}
        />
      </div>
    </div>
  )
  ...
  const mapItems = {
    ...
    'Saved & Notes': [
      <SavedItem
        index={1}
        callback={item => {
          setSavedActiveOpenFilter(item)
        }}
        activeOpenFilter={savedActiveOpenFilter}
        text="Odio purus ac ac sem vitae pulvinar viverra lacus, lectus. Cure pa lactus."
      />,
      <SavedItem
        index={2}
        callback={item => {
    ...
    Templates: [<TemplateItem />, <TemplateItem />, <TemplateItem />],
  }
...
const MapItemBottom = ({
  ...
  const mapItems: Record<string, string[]> = {
    Roadmap: ['All', 'Guide', 'Mission'],
    Activity: ['All', 'Service', 'Order', 'Sponsorship'],
    'Saved & Notes': ['All', 'User', 'Service'],
    Templates: ['All', 'Recent', 'Saved'],
  }
  const selectedItems = mapItems[activeSelection] || [] // Handle the case where activeSelection is not found
      ...
      <div className={styles.bottom_part_padding}>
        <div className={styles.bottom_buttons}>
          {selectedItems.map(item => (
            ...
            >
              <BottomFilterItem
                activeBottomSelect={activeBottomSelect}
                item={item}
              />
            </div>
          ))}
        ...
        <div className="gap_20">
          {activeSelection == 'Saved & Notes' && (
            <TextDropdownSelect
              text="NO LIST"
              variants={['NO LIST', '2', '3']}
            />
          )}
          {activeSelection == 'Templates' && (
            <TextDropdownSelect text="ORDERS" variants={['ORDERS', '2', '3']} />
          )}
          <DropdownNumber />
        </div>
      </div>
...
const BottomFilterItem = ({ item, activeBottomSelect }) => {
  ...
  return (
    ...
    >
      <Typography
        color={
          item == activeBottomSelect || hovered
            ? AppColor.orange
            : AppColor.text
        }
