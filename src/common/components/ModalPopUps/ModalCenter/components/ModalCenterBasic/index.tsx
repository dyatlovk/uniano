...
const ModalCenterBasic = ({
  ...
  const modalWidth =
    width + 20 <= parseInt(desktopMinWidth) ? 'calc(100vw - 20px)' : 'auto'
  const minWidthGetted =
    width + 20 <= parseInt(desktopMinWidth)
      ? 'calc(100vw - 20px)'
      : desktopMinWidth
  const maxWidthGetter =
    width + 20 <= parseInt(desktopMaxWidth)
      ? 'calc(100vw - 20px)'
      : desktopMaxWidth
    ...
    <ModalCenter onClickHandler={callbackClose}>
      <div
        style={{
          ...stylePass,
          width: modalWidth,
          minWidth: minWidthGetted,
          maxWidth: maxWidthGetter,
        }}
        className={styles.modal_center_basic}
      >
        <div
          className={styles.top_part}
          style={{ padding: topPartPadding ?? '20px 30px' }}
        >
          {prevClose && (
        ...
        <div
          ...
          style={{ padding: bottomPartPadding }}
        >
          {children}
        </div>
      </div>
