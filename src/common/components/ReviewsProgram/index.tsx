...
const ReviewsProgramCard = ({
  ...
  images,
  addInfo,
}: ReviewsProgramCardProps) => {
  const imagesArray =
    addInfo != null ? addInfo.users.map(item => item.image) : null
      ...
      {shareModal && (
        ...
        >
          <div className={styles.share_modal}>
            <AppColor.close
              className={`${styles.close_icon} cursor`}
              onClick={() => {
                setShareModal(false)
              }}
              width={'17px'}
    ...
    <div className={styles.image_row}>
      {images.length > 4 ? (
        <div>
          ...
          />
          <div
            style={{ left: `${1 * 33}px`, zIndex: '2' }}
            className={styles.images_length_show}
          >
            <Typography fontSizeStatic="10px" fontWeight="500">
              +{images.length - 3}
            </Typography>
          </div>
        </div>
