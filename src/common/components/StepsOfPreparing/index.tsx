  ...
  return (
    <div className={styles.steps_grid}>
      {elements.map((item, index) => (
        <StepItem
          drawLine={elements.length - 1 != index}
          solve={item.solve}
          text={item.text}
        />
      ))}
    ...
    <div className={styles.step_item}>
      ...
      <div className={styles.solve_part}>
        ...
        >
          ...
          <Typography
            variant="body5"
            className={styles.step_item_hover_text}
...
export const StepsOfPreparingEndSolving = ({
  elements,
  solvingNode,
}: StepsOfPreparingEndSolvingProps) => {
  return (
    <div className={styles.steps_grid}>
      {elements.map((item, index) => (
        <StepItem
          afterTextNode={item.afterTextNode}
          drawLine={item.drawLine ?? true}
          solve={item.solve}
          onSolveClick={item.onSolveClick}
          text={item.text}
        />
      ))}
      {solvingNode}
