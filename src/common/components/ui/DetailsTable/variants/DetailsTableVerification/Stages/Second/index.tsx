...
const Photo = ({ title, imgPath, status }: PhotoProps): JSX.Element => {
  const findStatusColor = (status: VerifyStatus): string => {
    const found = Statuses.find(el => el.status === status)
