...
const DropdownNodeActivity = ({
  activityItems,
  countNotification,
  filters,
}: DropdownNodeActivityProps) => {
      ...
      dropnodes={
        <div className={styles.node_wrapper}>
          {(() => {
            ...
            for (
              let index = 0;
              index < activityItems.length && index + 1 <= showItemsCount;
              index++
            ) {
      ...
      <div className={styles.top_part}>
        ...
        <div>
          ...
          <Typography variant="body4">
            ...
            {tag.map(item => (
              <span className={styles.hover_text}> • {item}</span>
            ))}{' '}
            • <span className={styles.hover_text}>{present}</span>
          </Typography>
        </div>
