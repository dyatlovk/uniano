...
export const ButtonDropdownSelect = ({
  text,
  variants,
}: ButtonDropdownSelectProps) => {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div
        ...
        style={{
          backgroundColor: openDropdown || hovered ? AppColor.orange : 'white',
        }}
        className={`${styles.button_active} cursor`}
      >
        <Typography
          textTransform="uppercase"
          variant="body4"
          fontWeight="500"
          color={openDropdown || hovered ? 'white' : AppColor.orange}
        >
          {activeSelection}
        </Typography>
        {openDropdown ? (
          ...
          <AppColor.chevronBottom
            fill={hovered ? 'white' : AppColor.orange}
            width={'16px'}
            height={'8px'}
          />
        )}
      ...
      <div
        ...
        className={styles.absolute_item}
      >
        <div style={{ position: 'relative' }}>
          <div className={styles.items_grid}>
            {variants.map(item => (
...
export const ButtonDropdownSelectDark = ({
  text,
  variants,
}: ButtonDropdownSelectProps) => {
  const [openDropdown, setOpenDropdown] = useState(false)
...
export const TextDropdownSelect = ({
  text,
  variants,
}: ButtonDropdownSelectProps) => {
  const [openDropdown, setOpenDropdown] = useState(false)
    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        <Typography
          ...
          fontWeight="500"
          color={
            openDropdown || hovered ? AppColor.text : AppColor.transparentBlack
          }
        >
          {activeSelection}
        </Typography>
