...
const TimeChooseDropdown = () => {
  ...
  return (
    <div ref={dropdownRef} style={{ position: 'relative' }}>
      <div
        style={{ ...borderStyles }}
        className={`${styles.time_choose} cursor`}
        onClick={() => {
          setShowDropdown(prev => !prev)
        }}
      >
        <Typography color={AppColor.transparentBlack} variant="body5">
          {daysOff ? 'Day off' : `${selectedTimeFrom} - ${selectedTimeTo}`}
        </Typography>
      ...
      <div
        ...
        className={styles.dropdown_absolute}
      >
        ...
        <div
          style={{ display: !daysOff ? 'grid' : 'none' }}
          className={styles.time_main_grid}
        >
          <div
            className={`${styles.time_items_grid} ${styles.item_grid_border}`}
          >
            {timeArray.map(item => (
              <div
                className={`${styles.time_item} cursor`}
                onClick={() => {
                  if (daysOff) {
                    setDaysOff(false)
                  }
              ...
              >
                <Typography
                  fontWeight={item == selectedTimeFrom ? '500' : '400'}
                  variant="body5"
                >
                  {item}
                </Typography>
          ...
          <div className={styles.time_items_grid}>
            {timeArray.map(item => (
              ...
              >
                <Typography
                  fontWeight={item == selectedTimeTo ? '500' : '400'}
                  variant="body5"
                >
                  {item}
                </Typography>
