    ...
    case 'row':
      return (
        <UserAvatarRow
          gap={gap}
          title={title}
          activeAgoRole={activeAgoRole}
          roleColor={roleColor}
          noWrap={noWrap}
          nodeAfterText={nodeAfterText}
          activeAgo={activeAgo}
          width={width}
          height={height}
          active={active}
...
const UserAvatarImage = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`${styles.active_status} ${active ? styles.active_true : styles.active_false}`}
      ></div>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div className={styles.position_relative}>
        {url != null ? (
          <img
            width={width != null ? width : '38px'}
            height={height != null ? height : '38px'}
            src={url}
            alt=""
          />
        ) : (
      ...
      <div
        style={
          preventMobileNone
            ? { display: 'block', whiteSpace: noWrap ? 'nowrap' : 'pre-wrap' }
            : {}
        }
      ...
      >
        ...
        <Typography
          color={roleColor ?? AppColor.orange}
          variant="body5"
          textLineHeight="1"
        >
          {role}
          {activeAgoRole != '' && (
            <span>
              {role && <span style={{ color: AppColor.text }}> • </span>}
    ...
    <div className={styles.wrapper}>
      ...
      <div style={stylesItem} className={styles.flex_wrapper}>
        ...
        <Typography color={AppColor.orange} textLineHeight="100%">
          {money != null ? `$${money}` : <AppColor.hideEye />}
        </Typography>
      </div>
    </div>
  )
