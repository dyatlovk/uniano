  ...
  return (
    <div style={{ position: 'relative' }}>
      <div
        ...
        style={{ ...borderStyles, transition: '0.2s' }}
        className={styles.border_input}
      >
        <label htmlFor="example">
          {borderText}{' '}
          {isRequired && <span className={styles.red_text}>*</span>}{' '}
        </label>
      ...
      <div className={styles.variants_wrapper} style={{ display: 'grid' }}>
        <AnimateHeight show={showDropdown}>
          ...
          {dropdownVariantsNodes &&
            dropdownVariantsNodes.map(item => (
              <div
                className={`${styles.node_select} cursor`}
                onClick={() => {
                  setActiveDropdownNode(item)
                  if (callback) {
                    callback(item.text)
                  }
          ...
          {dropdownVariants &&
            dropdownVariants.map(item => (
              <div
                ...
                className={`${styles.variant_item} cursor`}
              >
                {' '}
