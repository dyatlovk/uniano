...
const PostCard = ({
  ...
  return (
    <div
      ...
      style={{
        backgroundColor:
          indexInParent % 2 == 0 ? AppColor.white : 'transparent',
      }}
    >
      <div
        onClick={() => {
        ...
        style={{ display: isPined ? 'block' : 'none' }}
      >
        <AppColor.pinned_true />
      </div>
      <div
        onClick={() => {
          setIsPined(prev => !prev)
        ...
        className={styles.pinned}
        style={{ opacity: !isPined && hovered ? '1' : '0' }}
      >
        <AppColor.pinned_process />
      </div>
      <div className={styles.text_wrapper}>
        <div className={styles.flex_center}>
          ...
          {isPined && isDesktop ? (
            ...
            <AppColor.heart width={'0px'} color="transparent" />
          )}
          {created && isDesktop && <AppColor.pause />}
          {comments.length > 0 && isDesktop && (
            <div className={styles.flex_center_min}>
              <AppColor.message width={'18px'} height={'18px'} />{' '}
              {comments.length}
            </div>
          )}
        </div>
