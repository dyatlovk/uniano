    ...
    <div>
      {blockedModal && (
        ...
        >
          ...
          <Typography variant="body4">
            • Prevent them from any activity towards to you: messages, project,
            and etc..
          </Typography>
          <SizeBox height="10px" />
          ...
          <InputCommon
            ...
            disableClose={true}
            endNode={<DarkBox onClick={() => {}} text="ADD" />}
          />
          <DynamicPadding desktop="30px" mobile="20px" />
      ...
      {passwordModal && (
        <ModalCenterBasic
          ...
          title={
            passwordStage == 1 ? 'Change Password' : 'Confirm your password'
          }
        >
          {passwordStage == 1 && (
            <>
...
const PasswordScore = ({ password }: PasswordScoreProps) => {
  const mapItems: Map<boolean, scoreSystem> = new Map([
    ...
    [
      password.length < 10 && password.length >= 8,
      { title: 'Try a few random words.', color: '#F2C94C', score: 'Weak' },
    ],
    [
      password.length >= 10 && password.length < 13,
      {
        title: 'That password is good, but you can make it even better.',
        color: AppColor.orange,
        score: 'Fair',
      },
    ],
    [
      password.length >= 13,
      { title: 'Great password !', color: AppColor.green, score: 'Strong' },
    ],
  ])
  ...
  const percent =
    currentScore.score == 'Error'
      ? 0
      : currentScore.score == 'Weak'
        ? 25
        : currentScore.score == 'Fair'
          ? 50
          : 100
  return (
    <div className="flex_space_between">
      <Typography variant="body5" fontWeight="500" color={currentScore.color}>
        * {currentScore.title}
      </Typography>
  ...
  return (
    <div>
      ...
      <Typography variant="body4">
        ...
        ensures that only devices you trust are able to access your account.
        Whenever a new device attempts to login to your account, you will be
        required to confirm the login by using a code sent to your email address
        or phone number.
      </Typography>
      ...
      <div className={styles.grid_20}>
        <ItemFactor
          text={
            contryNumber != '' && number != ''
              ? formatPhoneNumber(`${contryNumber}${number}`)
              : 'No phone number'
          }
        ...
        />
        <ItemFactor
          text={pin.length > 0 ? '******' : 'No 6-digit PIN'}
          title={'PIN'}
          onClick={() => {}}
        />
      </div>
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div
        ...
        className={styles.abs_item}
      >
        <AnimateHeight show={showDropdown}>
          <div>
            {countries.map(item => (
              <div
                className={`${styles.dropdown_item_number} cursor`}
                onClick={() => {
                  setCountry(item)
